...
async function getStats() {
  ...
  const [orders, users, subs, flagged] = await Promise.all([
    ...
    supabase.from('stride_sessions').select('id, user_id, steps, flag_reason').eq('is_flagged', true).is('reviewed_at', null),
  ])
      ...
      {stats.flaggedSessions > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-5 py-3 flex items-center justify-between">
          <span className="text-amber-800 text-sm font-medium">
            ⚠️ {stats.flaggedSessions} flagged Stride session{stats.flaggedSessions > 1 ? 's' : ''} need review
          </span>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <a
              key={a.href}
              href={a.href}
              className="px-4 py-2 rounded-lg bg-[#f5f5f5] hover:bg-[#FF6357] hover:text-white text-sm font-medium text-[#1a1a2e] transition-colors"
            >
              {a.label}
            </a>
          ))}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {stats.recentOrders.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-5 py-8 text-center text-gray-400">No orders this month yet</td>
                </tr>
                ...
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right font-medium text-[#1a1a2e]">${(order.total_usd ?? 0).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
