...
export default async function AdminOrders() {
  ...
  const { data: orders } = await supabase
    .from('orders')
    .select('id, created_at, status, total_usd, currency, user_id, profiles(full_name, email)')
            ...
            <tbody className="divide-y divide-gray-100">
              {(orders ?? []).length === 0 ? (
                ...
                <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColor[o.status] ?? 'bg-gray-100 text-gray-500'}`}>
                      {o.status}
                    </span>
                  </td>
