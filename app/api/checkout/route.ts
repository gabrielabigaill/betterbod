...
export async function POST(req: Request) {
  ...
  if (entry) {
    if (now < entry.reset) {
      ...
      entry.count++
    } else { ipAttempts.set(ip, { count: 1, reset: now + 60_000 }) }
  } else { ipAttempts.set(ip, { count: 1, reset: now + 60_000 }) }

  let body
  try { body = CheckoutSchema.parse(await req.json()) }
  ...
  const total = PLAN_PRICES[body.currency][body.planType].toFixed(2)
