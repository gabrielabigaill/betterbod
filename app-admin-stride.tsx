...
export default async function AdminStride({ searchParams }: { searchParams: { filter?: string } }) {
  ...
  let query = supabase
    ...
    .select('id, user_id, steps, duration_seconds, started_at, ended_at, is_flagged, flag_reason, week_number, year, profiles(full_name, email)')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <a href="/admin/stride"
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${!flaggedOnly ? 'bg-[#FF6357] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
          All Sessions
        </a>
        <a href="/admin/stride?filter=flagged"
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${flaggedOnly ? 'bg-[#FF6357] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
