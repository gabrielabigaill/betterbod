...
const FALLBACK_RATES: Record<string, number> = {
  USD: 1, JMD: 158, AUD: 1.52, GBP: 0.79, EUR: 0.92, CAD: 1.37, NZD: 1.63, TTD: 6.79,
}

const CURRENCY_SYMBOLS: Record<string, string> = {
  USD: '$', JMD: 'J$', AUD: 'A$', GBP: '£', EUR: '€', CAD: 'C$', NZD: 'NZ$', TTD: 'TT$',
}
...
export function CurrencyProvider({
  children, initialCode, initialSymbol, initialCountry,
}: {
  ...
  const format = useCallback((usdAmount: number, decimals = true) => {
    ...
    return decimals ? `${sym}${converted.toFixed(2)}` : `${sym}${Math.round(converted).toLocaleString()}`
  }, [code])
