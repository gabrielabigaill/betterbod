...
const StopSchema = z.object({
  ...
  steps:            z.number().int().min(0).max(50_000),
...
export async function POST(req: Request) {
  ...
  // Verify active Stride Club subscription
  ...
  if (body.action === 'start') {
    ...
    const { data, error } = await supabase
      ...
      .insert({ user_id: user.id, steps: 0, started_at: now.toISOString(), week_number: weekNumber, year })
      .select('id')
      .single()
  ...
  if (body.action === 'stop') {
    ...
    if (sessionAge < 60) return NextResponse.json({ error: 'Session too short' }, { status: 400 })
    ...
    const flagReason     = stepsPerSecond > 5
      ...
      : parsed.steps >= 50_000 ? 'Max steps cap reached' : null

    const { error } = await supabase
      .from('stride_sessions')
      .update({ steps: parsed.steps, duration_seconds: parsed.duration_seconds,
                ended_at: new Date().toISOString(), is_flagged: isFlagged, flag_reason: flagReason })
      .eq('id', parsed.session_id)
...
// GET — weekly leaderboard (public within stride club)
