...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f5f5]">
      <aside className={clsx('flex flex-col bg-[#1a1a2e] text-white transition-all duration-300 shrink-0', collapsed ? 'w-16' : 'w-60')}>
        <div className="flex items-center justify-between px-4 py-5 border-b border-white/10">
          ...
          <button onClick={() => setCollapsed(c => !c)} className="ml-auto text-white/50 hover:text-white transition-colors p-1 rounded" aria-label="Toggle sidebar">
            {collapsed ? '→' : '←'}
          </button>
        </div>
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={clsx('flex items-center gap-3 px-4 py-2.5 text-sm transition-colors',
                  active ? 'bg-[#FF6357]/20 text-[#FF6357] border-r-2 border-[#FF6357]' : 'text-white/60 hover:text-white hover:bg-white/5')}
        ...
        <div className="border-t border-white/10 p-4">
          <Link href="/" className="flex items-center gap-3 text-sm text-white/40 hover:text-white transition-colors" title={collapsed ? 'View Site' : undefined}>
